...
export default function Header({ totalProfit }: HeaderProps) {
  ...
  const profitColor = isLoss ? 'text-[#FF3B30]' : 'text-[#3082ff]';
    ...
    <header className="shrink-0">
      <div className="flex items-center justify-between pl-2 pr-4 py-3">
        ...
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="icon" className="h-12 w-10 text-muted-foreground hover:bg-transparent focus-visible:ring-0 focus-visible:ring-offset-0 active:bg-transparent">
            <SortIcon />
          </Button>
          <Button variant="ghost" size="icon" className="h-12 w-10 text-muted-foreground hover:bg-transparent focus-visible:ring-0 focus-visible:ring-offset-0 active:bg-transparent">
            <AddOrderIcon />
